...
async function fetchGiveawaysData(query = '') {
    try {
        const response = await fetch(`https://gfg-proxy.vercel.app/api/giveaways${query}`);
        if (!response.ok) {
...
if (giveaway.platforms && typeof giveaway.platforms === 'string') {
    const platforms = giveaway.platforms.split(',').map(platform => platform.trim());
    createPlatformIcons(platforms, platformIconsContainer);
} else {
    console.error('Giveaway object or its platforms property is undefined or not a string.');
}
badgePositionSpan.appendChild(platformIconsContainer);
    ...
    const thumbnailImage = createThumbnailImage(giveaway.thumbnail, giveaway.title);
...
function createTimeLeftBadge(endDate) {
    ...
    if (daysRemaining >= 0) {
        if (daysRemaining >= 31) {
            ...
            badgeText = `${monthsRemaining} month${monthsRemaining > 1 ? 's' : ''} left`;
        } else if (daysRemaining > 0) {
            badgeText = `${daysRemaining} day${daysRemaining > 1 ? 's' : ''} left`;
        } else if (hoursRemaining > 0) {
            badgeText = `${hoursRemaining} hour${hoursRemaining !== 1 ? 's' : ''} left`;
        }
    ...
    timeLeftBadge.classList.add('badge', 'giveaway__badge', 'badge--pill', 'giveaway__badge--time-left', badgeClass);
...
function createSection(giveaway) {
    ...
    badges.appendChild(createBadge('FREE', 'badge--free', 'giveaway__original-price', 'text-muted'));
    ...
    usersClaimed.innerHTML = `<i class="fas fa-users"></i> <strong>${giveaway.users ? `${giveaway.users} Claimed this loot!` : 'Be the first to claim!'}</strong>`;
...
function createTypeBadge(type) {
    ...
    switch (type) {
        ...
        case 'other':
            ...
            break;
        default:
            typeBadge.textContent = type ? type.charAt(0).toUpperCase() + type.slice(1) : ''; // Capitalize first letter of unknown type
    }
    return typeBadge;
}
...
function createClaimGameButton(url) {
    ...
    claimGameButton.classList.add('btn', 'btn-outline-blue', 'giveaway__button', 'giveaway__button--claim-loot');
    ...
    platforms.forEach(platform => {
        ...
        if (iconClassName && !displayedIcons.has(iconClassName)) {
            ...
            if (platform === 'Nintendo Switch') {
                ...
                platformIcon.style.backgroundImage = 'url("assets/images/icons/switch.png")';
            } else {
